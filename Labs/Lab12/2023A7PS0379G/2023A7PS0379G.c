...
int smallest_name_vowel_order(org *o)
{
    ...
    int vowel_indices[minLength], num_vowels = 0;

    for (int i = 0; i < minLength; i++)
    {
        char ch = o->e[minIndex]->fname[i];
        if (isVowel(ch))
        {
            int correct_index_in_vowel_indices = num_vowels;
            vowel_indices[num_vowels++] = i;

            for (int j = correct_index_in_vowel_indices - 1; j >= 0; j--)
            {
                char ch_at_jth = o->e[minIndex]->fname[vowel_indices[j]];
                if (!(islower(ch_at_jth) && isupper(ch)) && ((islower(ch) && isupper(ch_at_jth)) || (ch < ch_at_jth)))
                {
                    correct_index_in_vowel_indices = j;
                    o->e[minIndex]->fname[vowel_indices[j+1]] = ch_at_jth;
                }
                else
                    j = -1; // To break
            } 
            o->e[minIndex]->fname[vowel_indices[correct_index_in_vowel_indices]] = ch;
        }        
    }
