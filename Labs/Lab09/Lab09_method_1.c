...
int num_names(struct student s[], int len, char key)
{
  ...
  for (int i = 0; i < len; i++)
  {
    if (toupper(s[i].name[0]) == toupper(key))
      count++;
  }
...
/* ------------------------------------------------------------------------------
 ...
 Helper functions used:
    * int CheckCorrectIndex(struct student students[], int current_index)
    * void ShiftElements(struct student students[], int n, int correct_index, int current_index)
---------------------------------------------------------------------------------*/

void ShiftElements(struct student students[], int n, int correct_index, int current_index)
{
  struct student student_to_replace = students[current_index];

  for (int i = current_index; i > correct_index; i--)
    students[i] = students[i - 1];
  
  students[correct_index] = student_to_replace;  
}
...
int CheckCorrectIndex(struct student students[], int current_index)
{
  ...
  {
    if (students[current_index].total > students[i - 1].total)
      correct_index = i - 1;        
  }
...
void sort_descending(struct student s[], int len)
{
  for (int i = 1; i < len; i++)
    {
      int correct_index = CheckCorrectIndex(s, i);
      if (correct_index != i)
        ShiftElements(s, len, correct_index, i);
