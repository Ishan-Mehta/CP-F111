...
int smallest_name_palindrome(org *o)
{
    ...
    //Going from the largest possible length to 3 or whenever a palindrome is found
    for(int i = strlen(o->e[minIndex]->fname); (i >= 3 && flag == 0); i--)
    {
        int index = 0;
        //Finding all possible substrings of the current length (i)
        while((index + i - 1) < strlen(o->e[minIndex]->fname) && (flag == 0))
        {
            char temp[50];
            for(int j = 0; j < 50; j++)
                temp[j] = '\0';
            
            //Making temporary substrings from the original string
            int count = 0;
            for(int j = index; j <= (index + i - 1); j++)
                temp[count++] = o->e[minIndex]->fname[j];
            
            
            //Check whether the temporary substring is palindrome or not
            int length = strlen(temp);
            int temp_flag = 0;

            for (int i = 0; (i < length / 2 && temp_flag == 0); i++) 
            {
    ...
    if(flag == 0)//No palindromic string of at least 3 characters were found
    {
        int len = strlen(o->e[minIndex]->fname);

        for (int i = 0, j = len - 1; i <= j; i++, j--) {
            // swapping characters
            char c = o->e[minIndex]->fname[i];
            o->e[minIndex]->fname[i] = o->e[minIndex]->fname[j];
            o->e[minIndex]->fname[j] = c;
        }
    }
