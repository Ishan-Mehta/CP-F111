...
void longest_word(char* s)
{
  ...
  for (int i = 0; i < strlen(s); i++)
  {
    ...
    else
    {
      ...
      else if (current_size == longest_size && current > longest) //strcmp(current, longest) > 0)
        strcpy(longest, current);
...
void incrementOrAppend(int number_of_leaves, int distinct_number_sick_leaves[], int frequencies[], int n)
{
  for (int j = 0; j < n; j++)
  {
    if (distinct_number_sick_leaves[j] == -1)
      distinct_number_sick_leaves[j] = number_of_leaves;

    if (distinct_number_sick_leaves[j] == number_of_leaves)
    {
      frequencies[j]++;
      j = n; // To break
    }
...
int findMode(int arr[], int x[], int n)
{
  int mode, highest_freq = 0;
  for (int i = 0; arr[i] != -1 && i < n; i++)
  {
    if (x[i] >= highest_freq)
    {
      highest_freq = x[i];
...
void cal_sick_leaves(int arr[], int n, int *mode, double *avg)
{
  *avg = 0;
  ...
  int distinct_number_sick_leaves[n], frequencies[n];
  for (int i = 0; i < n; i++)
  {
    distinct_number_sick_leaves[i] = -1;
    frequencies[i] = 0;
  }
  ...
  {
    *avg += arr[i];
    incrementOrAppend(arr[i], distinct_number_sick_leaves, frequencies, n);
  }

  *mode = findMode(distinct_number_sick_leaves, frequencies, n);
  *avg /= n;
}
