...
void longest_word(char* s)
{
  char longest[50]; 
  char current[50];
  int current_size = 0, longest_size = 0;

  for (int i = 0; s[i] != '\0'; i++)
  {
    if (s[i] != '_' && s[i] != '.')
      current[current_size++] = s[i];
    ...
    else
    {
      ...
      if (current_size == longest_size && strcmp(current, longest) > 0)
        strcpy(longest, current);
...
void IncrementOrAppend(int number_leaves, int *distinct, int *freq, int n)
{
  for (int i = 0; i < n; i++)
  {
    if (*(freq+i) == 0)
      *(distinct+i) = number_leaves;

    if (*(distinct+i) == number_leaves)
    {
      (*(freq+i))++;
      i = n;
    }
...
void cal_sick_leaves(int *arr, int n, int *mode, double *avg)
{
  *avg = 0;
  int distinct_sick_leaves[n], freq[n];
  for (int i = 0; i < n; i++)
    freq[i] = 0;
  ...
  {
    *avg += *(arr+i);
    IncrementOrAppend(*(arr+i), distinct_sick_leaves, freq, n);
  }
  *avg /= n;

  int highest_freq = 0;
  for (int i = 0; *(freq+i) != 0 && i < n; i++)
  {
    if (*(freq+i) >= highest_freq)
    {
      *mode = *(distinct_sick_leaves+i);
      highest_freq = *(freq+i);
    }
  }  
}
...
/**
 * @brief Prints the favourite quote of the employee with the higher salary by removing duplicate characters.
 * Do NOT ignore the case of the character.
 * Order of the characters should be the same as that of the original quote.
 * 
 * Requires: The quote will have '_' (underscore) as word separators and '.' (period/full stop) as the end of the quote.
 * Assume that words are non empty. That is, quote will not be "_."
 * Words contain only alphabets and digits.
 ...
 * Example 1:
 *    s= "BITS_Pilani."
 *    Prints: The quote removing duplicates: BITS_Pilan.
 * 
 * Example 2:
 *    s= "Its_Magic."
 *    Prints: The quote removing duplicates: Its_Magic.
 * 
 * Example 3:
 *    s= "Life_is_good."
 *    Prints: The quote removing duplicates: Life_sgod.
*/
void remove_dup(char* s)
{
  ...
  printf("The quote removing duplicates: %s\n", quote);
}
...
*/
  ...
  printf("\t3. Print the quote by removing duplicates.\n\n");
