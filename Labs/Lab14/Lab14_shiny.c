...
/**
 * @brief Returns the appropriate index for the given ASCII character in the ASCII to Braille 
 * map. Indexes 0-25 for letters 'a'-'z', 0-8 for digits '1'-'9', index 9 for digit '0', 26 
 * for ' ' (SPACE), and -1 for all other characters (that our translator does not handle).
 *
 */
...
/**
 * @brief Put the Braille-equivalent of the given character using the given map on the given 
 * file, if the character can be translated to Braille; ignores otherwise.
 *
...
void fputc_braille(char ch, FILE *fp, char *map[])
{
  ...
  {
    char *braille_line =ascii_to_braille(ch, map);
    fputc(braille_line[0],fp);
    fputc(braille_line[3],fp);
    fputc('\n',fp);
    fputc(braille_line[1],fp);
    fputc(braille_line[4],fp);
    fputc('\n',fp);
    fputc(braille_line[2],fp);
    fputc(braille_line[5],fp);
    fputc('\n',fp);
    fputc('\n',fp);
  }
}
...
/**
 * @brief Returns a FILE's handle after opening it in the given mode. Keeps prompting the 
 * user for a valid path otherwise.
 *
 */
...
/**
 * @brief Translates the contents of the given ASCII file to the Braille file using the given 
 * character map. Braille characters are printed on the `fp_braille` stream one below the 
 * other, separated by a blank line.
 *
 * Requires: fp_ascii is opened for reading, fp_braille is opened for writing, and map is 
 * properly populated
 *
 */
void translate_ascii_to_braille(FILE *fp_ascii, FILE *fp_braille, char *map[])
{
  ...
  int ch=fgetc(fp_ascii);
  while (ch!=EOF)
  {
    fputc_braille((char)ch, fp_braille, map);
    ch=fgetc(fp_ascii);
  }
}
